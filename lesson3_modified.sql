...
CREATE TABLE IF NOT EXISTS messages (
  from_profile_id BIGINT UNSIGNED NOT NULL COMMENT "Ссылка на отправителя сообщения",
  to_profile_id BIGINT UNSIGNED NOT NULL COMMENT "Ссылка на получателя сообщения",
...
CREATE TABLE IF NOT EXISTS relationships (
  from_profile_id BIGINT UNSIGNED NOT NULL COMMENT "Ссылка на инициатора дружеских отношений",
  to_profile_id BIGINT UNSIGNED COMMENT "Ссылка на получателя приглашения дружить",
  ...
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания строки",  
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT "Время обновления строки",  
  PRIMARY KEY (from_profile_id, to_profile_id) COMMENT "Составной первичный ключ",
...
CREATE TABLE IF NOT EXISTS communities_users (
  community_profile_id BIGINT UNSIGNED NOT NULL COMMENT "Ссылка на группу",
  user_profile_id BIGINT UNSIGNED NOT NULL COMMENT "Ссылка на пользователя",
  status_id TINYINT UNSIGNED NOT NULL COMMENT "User status in community",
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания строки",
  PRIMARY KEY (community_profile_id, user_profile_id, status_id) COMMENT "Составной первичный ключ",
...
CREATE TABLE IF NOT EXISTS media (
  ...
  user_profile_id BIGINT UNSIGNED COMMENT "Ссылка на пользователя, который загрузил файл",
  filename VARCHAR(255) NOT NULL COMMENT "Путь к файлу",
  filesize INT NOT NULL COMMENT "Размер файла",
...
CREATE TABLE IF NOT EXISTS  likes_media (
  profile_id BIGINT UNSIGNED NOT NULL COMMENT "ссылка на user",
  media_id BIGINT UNSIGNED NOT NULL COMMENT "ссылка на файл",
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "дата создания", 
  PRIMARY KEY (profile_id, media_id) COMMENT "Составной ключ",
